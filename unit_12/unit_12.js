...
function f12() {
    ...
    for (let i = 0; i < a12.length; i++) {
        for (let k = 0; k < a12[i].length; k++) {
            if (a12[i][k]) res.push(a12[i][k]);
        }
    }

    document.querySelector('div.out-12').innerHTML = res.join(' ');
}
...
function f14() {
    ...
    for (let i = 0; i < a14.length; i++) {
        arrLength.push(a14[i].length);
    }

    document.querySelector('div.out-14').innerHTML = arrLength.join(' ');
}
...
function f15() {
    document.querySelector('div.out-15').innerHTML = a15.sort((a, b) => b.length - a.length)[0].length;
}
